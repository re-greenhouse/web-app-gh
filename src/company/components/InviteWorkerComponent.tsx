...
export const InviteComponent = ({ hideDialog }: { hideDialog: () => void }): ReactElement => {
  const { usernameRef, firstNameRef, lastNameRef, emailRef, isLoading, onSubmit } = useRegisterForm();
    ...
    <div>
      <div className="absolute backdrop-blur-xs bg-black bg-opacity-25 inset-0 flex w-full justify-center h-full z-20 content-center items-center">
        <div className="bg-background lg:w-1/3 lg:h-2/3 content-start justify-center text-center rounded-2xl">
          <form onSubmit={(e) => onSubmit(e, isAdmin)} className="bottom-0 py-2">
            <div className="flex flex-col justify-center w-full p-5">
              ...
              <div className="text-start w-full">
                ...
                <div className="flex justify-between gap-3 py-5 lg:flex-row flex-col">
                  ...
                  <div className="flex flex-col w-full">
                    <h1>Rol</h1>
                    <select className="border-2 border-gray-300 rounded-md w-full p-2 mt-2" defaultValue="Tecnico" onChange={handleAdminChange}>
                ...
                <div className="flex flex-col p-10 gap-8 w-full">
                  <PrimaryButton disabled={isLoading} type="submit">
                    {isLoading ? <Spinner color="#15F5BA" height={24} /> : <span className="text-md">Enviar invitación por correo</span>}
                  </PrimaryButton>
                </div>
              </div>
            </div>
